# EGOS-KERNEL-PROPAGATED: 2026-08-09
<!-- AUTO-INJECTED by disseminate-propagator.ts — DO NOT EDIT THIS BLOCK MANUALLY -->
<!-- Kernel commit: ec937e7f | 1 rule section(s) changed -->
<!-- Source of rules: egos/AGENTS.md (canonical). Kernel-only authoritative copy: ~/.claude/CLAUDE.md -->
<!-- Re-run: bun ~/egos/scripts/disseminate-propagator.ts --all to update -->
<!-- ~ AGENTS.md → ### P2 — HUMANO SOBERANO (2 lines) -->

> ⚠️ **PROPAGATED FROM KERNEL** — Edits to this block are overwritten by next `bun governance:sync:exec`.
> Edit kernel `egos/AGENTS.md` section between `<!-- PROPAGATE-RULES-BEGIN -->` and `<!-- PROPAGATE-RULES-END -->` instead.
>
> 🧭 **COMO LER OS PONTEIROS DESTE BLOCO (R-PONTEIRO-001).** Todo caminho relativo aqui — `docs/…`, `scripts/…`, `.husky/…` — é **relativo ao kernel EGOS**, não a este repositório. Se não resolver aqui, procure no clone do kernel (`~/egos` no Linux · `C:\Users\%USERNAME%\egos` no Windows) **antes** de concluir que a regra não existe. Não estando em nenhum dos dois, é fantasma: registre como tal — **nunca invente a regra que "deveria" estar lá.**
...
**REFLEXOS PRÉ-AÇÃO (moldam a decisão ANTES de existir artefato; exceção auditável do mapa):** **R-ARCH-001** — decisão que é do CLIENTE (vendor/preço/prazo/stack/canal) nunca inferida: placeholder + trade-off; cliente escolhe no diagnóstico · **R-DIAG-001** — diagnóstico antes de demo; no máximo demo GENÉRICO pós-1ª-entrevista · **Orquestração** (main-loop Opus/Fable, por turno): Edit>80L / Bash longo / 5+ leituras → `Agent` com `model` EXPLÍCITO (haiku=mecânico · sonnet=raciocínio · Opus/Fable NUNCA subagente); ajo direto só em edição ≤10L, fix de gate, decisão/commit, texto constitucional. **Em `Workflow` isso virou FREIO (R-MODEL-DELEG-001, M1 2026-07-26):** `agent()` sem `model:` é RECUSADO no despacho pelo hook `workflow-model-guard` — antes era só prosa, e por isso 48 subagentes saíram todos em Opus em 25/07 e mais 14 em 26/07. Na dúvida entre dois modelos, escolha o menor: subir custa uma re-execução, descer custa dinheiro já gasto · **Git destrutivo** — antes de merge-divergente/rebase-amplo/purga: `git bundle --all` + verify + clone-teste PROVADO; janela alheia no status → auto-isolamento path-scoped · **PERIMETER-FIRST (R-PERIM-001/002)** — nada se constrói fora de repo git com perímetro ativo; deploy só de tree commitada; máquina-mãe roda checks do kernel VIVO · **Contexto** — pós-autocompact = MESMA janela; 🟡84% → `/compact`; 10+ turnos → releia a task · **Copy pública** — absolutos banidos ("100%", "perfeito", "garantido", "único/primeiro do Brasil") → refutar e substituir por prova. Corpos: `egos/docs/governance/EGOS_OPERATING_PRINCIPLES.md` + `MODEL_DELEGATION_POLICY.md`.
...
> 
> A Fonte Única de Verdade para o ecossistema de Agentes do EGOS foi movida para o repositório **`egos`** (O Laboratório de Agentes).
> As regras de engajamento e meta-prompts agora vivem no pacote `@enioxt/ai-rules`.
